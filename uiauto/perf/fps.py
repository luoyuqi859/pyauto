...
class SurfaceStatsCollector(object):
    ...
    def __init__(self, device, frequency, package_name, fps_queue, jank_threshold, path=None, use_legacy=False):
        self.device = device
    ...
    def start(self, start_time):
        ...
        if not self.use_legacy_method and self._clear_surfaceflinger_latency_data():
            try:
                ...
                logger.warning('无法动态获取当前Activity名称，使用page_flip统计全屏帧率！')
    ...
    def _collector_thread(self):
        """
        收集surfaceflinger数据用了两种方式:
        use_legacy_method
            * 为ture时，需要root权限:service call SurfaceFlinger 1013 得到帧数
            ...
            try:
                ...
                if self.use_legacy_method:
                    ...
                    if len(timestamps):
                        ...
                        # 两种情况：
                        # 1）activity发生变化，但旧的activity仍然存时，取的时间戳不为空，但时间全部小于等于last_timestamp
...
class FPSMonitor():
    ...
    def __init__(self, device_id=None, package_name=None, path=None, frequency=1.0, timeout=24 * 60 * 60,
                 fps_queue=None,
        ...
        self.fpscollector = SurfaceStatsCollector(device=self.device, path=self.path, frequency=self.frequency,
                                                  package_name=self.package, fps_queue=fps_queue,
                                                  jank_threshold=self.jank_threshold, use_legacy=self.use_legacy)
