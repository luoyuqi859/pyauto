...
from uiauto import ADB
from utils.log import logger
from utils.time_fun import timeoperator
from utils.tools import TimeUtils
...
class Monkey(object):
    ...
    def __init__(self, path=None, device_id=None, package=None, timeout=1):
        '''构造器
    ...
    def start_monkey(self, package, timeout):
        ...
        self._monkey_thread = threading.Thread(target=self._monkey_thread_func, args=[self.path])
...
if __name__ == "__main__":
    package = "com.gm.hmi.settings"
    monitor = Monkey(path=settings.root_path / "uiauto" / "perf" / "record", package=package)
