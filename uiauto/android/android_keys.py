...
@Author: Luo Yuqi
@File: android_keys
@Created: 2023/2/23 17:52
"""

from enum import IntEnum


class AndroidKeys(IntEnum):
    UNKNOWN = 0
    SOFT_LEFT = 1
    SOFT_RIGHT = 2
    HOME = 3
    ...
    @staticmethod
    def get_code(name):
        key_code = AndroidKeys.__members__.get(name.upper())
        return int(key_code) if key_code else 0

    @staticmethod
    def get_name(code):
        if isinstance(code, AndroidKeys):
            name = code.name
        elif isinstance(code, int):
            name = AndroidKeys(code)
        else:
            name = str(code)
