...
class Controller:
    ...
    async def swipe(self, x, y, end_x, end_y, unit=5, delay=1):
        ...
        end_x, end_y = min(end_x * self.device.resolution[0], self.device.resolution[0]), min(
            end_y * self.device.resolution[1], self.device.resolution[1])
        step = 1
        while True:
            if x_1 > end_x:
