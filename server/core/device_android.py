...
"""
@Author: Luo Yuqi
@File: device_android.py
@Created: 2023/4/26 15:56
"""
import socket
import subprocess
import sys
...
from server.core import download
...
class AndroidDevice(object):
    def __init__(self, serial: str, free_port: FreePort):
        ...
        self._serial = serial
        self._procs = []
        self._current_ip = settings.localhost_ip
    ...
    async def _init_forwards(self):
        """代理手机端口"""
        logger.debug("%s forward atx-agent", self.serial)
    ...
    async def proxy_device_port(self, device_port: int) -> tuple:
        ...
        server = subprocess.Popen([
            sys.executable, "-u", "core/proxy_port.py",
            "-lp", str(listen_port),
            "-tp", str(local_port)],
            stdout=sys.stdout)
